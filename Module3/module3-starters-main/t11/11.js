...
// add your code here
const sections = document.querySelector("#pictures");
const dialog = document.querySelector("dialog");
const modalImg = document.querySelector("#lgImg");
const closeBtn = document.querySelector("span");
for (let pic of picArray){
  sections.innerHTML += `<article class="card">
   <h2>${pic.title}</h2>
   <figure>
      <img src="${pic.image.medium}" alt="${pic.title}" data-large="${pic.image.large}">
...
function opened(evt){
  ...
  if (target && target.dataset.large) {
    modalImg.src = target.dataset.large;
...
sections.addEventListener("click", opened);
